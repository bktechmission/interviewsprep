...
public class BinaryTreeExamples {
	...
	static void binaryTreeExamples() {
		...
		BTNode root3 = constructBinarySearchTreeFromInAndPre(inList.toArray(new Integer[0]),
				preList.toArray(new Integer[0]), 0, preList.size() - 1);
		BTreePrinterBT.printNode(root3);

		posIndex = posList.size() - 1;
		BTNode root4 = constructBinarySearchTreeFromInAndPos(inList.toArray(new Integer[0]),
				posList.toArray(new Integer[0]), 0, posList.size() - 1);
		BTreePrinterBT.printNode(root4);

		System.out.println("Identical Tress root2, root3 and root4 : " + isIdenticalTree(root2, root3) + "  , "
				+ isIdenticalTree(root2, root4));
	...
	static int getMaxSum(BTNode root) {
		...
		int sumtoRet = Math.max(leftSum + root.data, Math.max(rightSum + root.data, root.data));

		globalmax = Math.max(globalmax, (leftSum + rightSum + root.data));

		return sumtoRet > 0 ? sumtoRet : 0;
	...
	static void createBSTFromSrotedLinkedList() {
		...
		headShared = head;
		BTNode root = bstFromSortedLinkedList(0,sizeOfLinkedList(head)-1);
	...
	static BTNode bstFromSortedLinkedList(int start, int end) {
		if (start>end)
			return null;

		int mid = (start+end)/2;
		BTNode left = bstFromSortedLinkedList(start, mid-1);
		BTNode root = new BTNode(headShared.data);
		root.left = left;
		headShared = headShared.next;
		root.right = bstFromSortedLinkedList(mid+1,end);
		return root;
	}
	...
	static void printAllNodesAtLevelWithKey(BTNode root, int key, int level, int curLevel, BTNode parent) {
		...
		if (curLevel == level && root.data != key && (parent.right == null || parent.right.data != key)
	...
	static boolean findAndReturnListWithAncestor(BTNode root, int key, List<BTNode> finalResult) {
		...
		if (findAndReturnListWithAncestor(root.left, key, finalResult)
				|| findAndReturnListWithAncestor(root.right, key, finalResult)) {
			return true;
		}
	...
	static void printLeftTopToBottom(BTNode root) {
		...
		System.out.print(root.data + ", ");
		if (root.left != null)
			printLeftTopToBottom(root.left);
		else
			printLeftTopToBottom(root.right);
